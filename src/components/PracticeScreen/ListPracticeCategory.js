...
import { CategoryButton } from '../MainScreen/CategoryButton';
import { firebase } from '../../firebase/config';
import firestore from '@react-native-firebase/firestore';
import { FlatList, Flex, View } from 'native-base';
import { useNavigation } from '@react-navigation/native';

export const ListPracticeCategory = () => {
    ...
    return (
        ...
        <FlatList
            ...
            numColumns={2}
            renderItem={(category) => <CategoryButton key={category.item?.id} id={category.item?.id} imgSrc={category.item?.imgUrl} title={category.item?.title} onPress={() => navigation.navigate('word', { categoryId: category.item?.id })} bg='lime.200'/>}/>
    )
}